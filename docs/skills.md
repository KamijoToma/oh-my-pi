...
- appends invocation metadata (see the identification/tagging formats below)
...
Invoked skill content is identified by invocation kind, each with its own prompt template (in `src/prompts/skills/`):
