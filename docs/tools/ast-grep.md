...
| `path` | `string` | No | One file, directory, glob, or internal URL with a backing file — or several of those as a semicolon-delimited list (`"src; tests"`). Omitted or empty defaults to `.` (the workspace root). Empty entries are rejected. Globs are forbidden for internal URLs. |
...
## Flow
1. `AstGrepTool.execute()` validates `pat`, normalizes `skip`, then delegates path resolution to `resolveToolSearchScope()` in `packages/coding-agent/src/tools/path-utils.ts`, which normalizes and rejects empty `path` entries.
...
## Errors
- TS wrapper throws `ToolError` for empty patterns, invalid `skip`, empty path entries, external (`http`/`https`/`ftp`/`ws`/`wss`) URLs (`file://` is intentionally accepted and converted to a local path), unsupported internal-URL globs, internal URLs without `sourcePath`, and missing paths.
