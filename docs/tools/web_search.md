...
| `limit` | `number` | No | Max results to return. Usually becomes the provider request's result-count parameter when `num_search_results` is absent. TinyFish uses it for paginated fetches before slicing; xAI enforces it only locally, clamping `numSearchResults ?? limit` to `1..30` (default `10`) to cap parsed sources/citations. |
...
| `num_search_results` | `number` | No | Requested search breadth or local result cap. Most providers send it upstream. TinyFish clamps to `1..20` with default `10`, sends it as `num_results` per page, and uses paginated fetches before slicing. xAI applies it as a local cap on parsed sources/citations — clamped to `1..30` with default `10` — and sends nothing result-count-related upstream. |
...
- **Provider adapters**
  ...
  - **xAI** — `packages/coding-agent/src/web/search/providers/xai.ts`
    ...
    - `max_tokens` and `temperature` pass through (`max_output_tokens` / `temperature`). The request carries no `search_parameters`: `recency` is ignored, and result counts are enforced locally only. Because xAI citations may include every encountered URL, the adapter caps returned `sources` and `citations` after parsing via `clampNumResults(...)` — `num_search_results` before `limit`, clamped to `1..30`, defaulting to `10` when omitted/invalid/zero.
...
- xAI local sources/citations cap (nothing result-count-related is sent upstream): `num_search_results` before `limit`, omitted/invalid/zero => local default `10`, max `30` (`clampNumResults` in `packages/coding-agent/src/web/search/providers/xai.ts`).
...
- Most providers treat `limit` and `num_search_results` as the same number because adapters pass `params.numSearchResults ?? params.limit`. Perplexity preserves both concepts. TinyFish uses the collapsed value as a local cap, serializes `num_results` per page, and paginates with `page` when more results are needed. xAI sends no result count upstream; it applies the collapsed value locally after parsing to cap returned sources/citations (`10` default, `30` max).
